def build_excel_file(current_file, previous_file, date_difference):
	...
	for node_name in database[current_file]:
		...
		sheet.cell(row = current_row, column = 5).value = database[current_file][node_name]['capacity'] * database[current_file][node_name]['usage'] / 100.0
		...
		current_color = 'FF0000' if database[current_file][node_name]['usage'] > 90 else ('FFA500' if database[current_file][node_name]['usage'] > 80 else '00FF00')
		...
		try:
			...
			sheet.cell(row = current_row, column = 8).value = median * database[current_file][node_name]['capacity']
			if median == 0:
				sheet.cell(row = current_row, column = 9).value = 0
				sheet.cell(row = current_row, column = 10).value = 'Estável'
			else:
				value = (database[current_file][node_name]['capacity'] - (database[current_file][node_name]['capacity'] * database[current_file][node_name]['usage'] / 100.0)) / (median * database[current_file][node_name]['capacity'])
